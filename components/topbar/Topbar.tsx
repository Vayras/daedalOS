...
import ControlCenterMenu from "@/components/topbar/ControlCenterMenu";
...
export const TopBar = () => {
  ...
  return (
    <div className="w-full h-8 px-2 fixed top-0 flex justify-between text-sm text-white bg-gray-700/10 backdrop-blur-2xl shadow transition">
      <div
        ref={appleBtnRef}
      ...
      <div className="flex flex-row items-center md:gap-2  h-8 my-auto px-1 cursor-default text-center">
        ...
        <div ref={wifiBtnRef} className="md:block hidden" onClick={toggleWifiMenu}>
          {wifi ? (
        ...
        <span className="text-lg hover:bg-gray-100/30 px-1 h-6 cursor-default rounded flex items-center" onClick={toggleShowSpotlight}>
          <Icon icon="bx:search" />
        </span>
        <span className="hover:bg-gray-100/30 px-1 h-6 cursor-default rounded flex items-center" onClick={toggleControlCenter}>
          <CCMIcon size={16} />
        </span>
